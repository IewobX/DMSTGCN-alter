...
# 计算loss的时候，并不需要计算padding部分的0，因此我们需要用到掩码损失函数
...
def masked_rmse(preds, labels, null_val=np.nan):
    return torch.sqrt(masked_mse(preds=preds, labels=labels, null_val=null_val)) #返回一个新张量，包含输入input张量每个元素的平方根

def masked_mae(preds, labels, null_val=np.nan):
    if np.isnan(null_val): #判断是否为空值
        mask = ~torch.isnan(labels)
    else:
        mask = (labels != null_val)
    mask = mask.float()
    ...
    loss = loss * mask
    loss = torch.where(torch.isnan(loss), torch.zeros_like(loss), loss) #torch.where()函数的作用是按照一定的规则合并两个tensor类型。
    return torch.mean(loss)
