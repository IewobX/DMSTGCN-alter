...
def main(args):
    ...
    for i in range(start_epoch, args.epochs + 1):
        ...
        train_rmse = []
        tt1 = time.time()
        dataloader['train_loader'].shuffle()  # 将序列的所有元素随机排序
        for itera, (x, y, ind) in enumerate(dataloader['train_loader'].get_iterator()):
            # 迭代器，为给定的iterable的每个项生成一个元组（index，item）
            # enumerate() 函数用于将一个可遍历的数据对象(如列表、元组或字符串)组合为一个索引序列，同时列出数据和数据下标，一般用在 for 循环当中
            trainx = torch.Tensor(x).to(device)  # 返回具有指定 dtype 的张量
            trainx = trainx.transpose(1, 3)  # 将channels的维度和最后一个维度对调
        ...
        yhat = yhat[:realy.size(0), ...]  # 对yhat进行切片，…代表省略所有的：
        for i in [2, 5, 11]:
    ...
    # final test
    bestid = np.argmin(his_loss)  # 求最小值对应的索引
    engine.model.load_state_dict(torch.load(  # 将预训练的参数权重加载到新的模型之中
